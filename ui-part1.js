// Wasteland Market Terminal — ui-part1.js (рынок, склад, сделки, события, цели, обзор)
...
function switchScreen(screen) {
    ...
    if (screen === 'advisor') renderAdvisor();
}
...
function selectItem(name) {
    ...
    if (activeEvent) {
        eventOptions.forEach((opt, i) => {
            if (opt.includes('value="' + activeEvent.type + '"')) {
...
function renderItemGraph() {
    ...
    const maxVal = Math.max(...buys), minVal = Math.min(...buys);
    ...
    for (let i = 0; i <= 4; i++) {
        ...
        gridLines += `<text x="${pad - 4}" y="${y + 3}" fill="#555" font-size="8" text-anchor="end">${val.toFixed(1)}</text>`;
    }
    let points = '', dots = '';
    const n = buys.length;
    buys.forEach((b, i) => {
...
function submitEvent() {
    const start = document.getElementById('eventStart').value, end = document.getElementById('eventEnd').value || start, type = document.getElementById('eventType').value;
    if (!start) return;
    addEvent(start, end, type, ''); renderEvents(); initCalendar();
}
function renderEvents() {
    const list = document.getElementById('eventsList');
    if (!list) return;
    if (events.length===0) { list.innerHTML='<p style="color:#888;">Нет событий</p>'; return; }
    const emoji={factory:'🏭',rating_end:'⚔️',battlepass:'🎫',road:'🛣️',raven:'🐦‍⬛',workshop:'🔧',bounty:'🎯'}, tagClass={factory:'tag-factory',rating_end:'tag-rating',battlepass:'tag-battlepass',road:'tag-road',raven:'tag-raven',workshop:'tag-workshop',bounty:'tag-bounty'};
    list.innerHTML=events.map((e,i)=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid var(--border);font-size:0.8em;"><span><span class="tag ${tagClass[e.type]||''}">${emoji[e.type]||'📌'}</span> ${e.start} → ${e.end}</span><button class="delete-btn" onclick="deleteEvent(${i});renderEvents();initCalendar();">✕</button></div>`).join('');
}
function initCalendar() { const now=new Date(); if(!calendarYear){calendarYear=now.getFullYear();calendarMonth=now.getMonth();} renderCalendar(); }
function renderCalendar() {
    const container=document.getElementById('calendarWidget');
    if(!container)return;
    const firstDay=new Date(calendarYear,calendarMonth,1).getDay()||7, daysInMonth=new Date(calendarYear,calendarMonth+1,0).getDate(), today=new Date(), todayStr=today.toISOString().slice(0,10);
    ...
    for(let day=1;day<=daysInMonth;day++){const dateStr=`${calendarYear}-${String(calendarMonth+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`, hasEvent=events.some(e=>e.start<=dateStr&&e.end>=dateStr), isToday=dateStr===todayStr; let cls='calendar-day'; if(isToday)cls+=' today'; if(hasEvent)cls+=' has-event'; html+=`<div class="${cls}" onclick="document.getElementById('eventStart').value='${dateStr}'">${day}</div>`;}
    html+='</div>'; container.innerHTML=html;
}
function changeMonth(delta){calendarMonth+=delta;if(calendarMonth<0){calendarMonth=11;calendarYear--;}if(calendarMonth>11){calendarMonth=0;calendarYear++;}renderCalendar();}

// === ЦЕЛИ ===
function addGoal(){const text=document.getElementById('goalText').value.trim(),target=parseFloat(document.getElementById('goalTarget').value),current=parseFloat(document.getElementById('goalCurrent').value)||0;if(!text||isNaN(target)){alert('Заполни');return;}addGoal(text,target,current);document.getElementById('goalText').value='';document.getElementById('goalTarget').value='';document.getElementById('goalCurrent').value='';renderGoals();}
function renderGoals(){const list=document.getElementById('goalsList');if(!list)return;if(goals.length===0){list.innerHTML='<p style="color:#888;">Нет целей</p>';return;}list.innerHTML=goals.map((g,i)=>{const pct=Math.min(100,(g.current/g.target)*100);return`<div class="item-card"><div class="name">🎯 ${g.text}</div><div style="font-size:0.8em;">${g.current.toFixed(2)} / ${g.target.toFixed(2)} ₽</div><div style="height:6px;background:#1a2a2a;border-radius:3px;margin-top:4px;"><div style="height:100%;width:${pct}%;background:var(--accent);border-radius:3px;"></div></div><button class="delete-btn" onclick="deleteGoal(${i});renderGoals();">✕</button></div>`;}).join('');}

// === ОБЗОР РЫНКА ===
function renderMarket(){const container=document.getElementById('marketContent');if(!container)return;let filtered=items;if(marketTab==='resources')filtered=items.filter(i=>i.type==='resource');if(marketTab==='parts')filtered=items.filter(i=>i.type==='part');if(filtered.length===0){container.innerHTML='<p style="color:#888;">Нет предметов</p>';return;}let html='';filtered.forEach(item=>{const data=prices[item.name]||[];if(data.length<2)return;const pred=getPrediction(item.name);html+=`<div class="item-card" onclick="switchScreen('items');selectItem('${item.name}');"><div class="name">${item.type==='resource'?'⛏️':'🔧'} ${item.name}</div><div style="display:flex;gap:8px;align-items:center;margin-top:4px;"><span style="font-size:0.8em;">${pred.avgBuy.toFixed(2)} ₽/шт</span><span class="status-badge ${pred.class}" style="font-size:0.65em;padding:2px 6px;">${pred.slope>0.1?'📈':pred.slope<-0.1?'📉':'📊'} ${pred.slope.toFixed(2)}/ч</span></div></div>`;});container.innerHTML=html||'<p style="color:#888;">Недостаточно данных</p>';}
